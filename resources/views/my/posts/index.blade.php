<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-ink">我的文章</h1>
        ...
        <div class="bg-white rounded-card border border-hairline overflow-hidden">
            @forelse($posts as $post)
                <div class="flex items-center justify-between p-4 {{ !$loop->last ? 'border-b border-hairline' : '' }}">
                    <div class="flex-1 min-w-0">
                        <h3 class="text-sm font-medium text-ink truncate">
                            <a href="{{ $post->status === 'published' ? route('posts.show', $post) : '#' }}" class="hover:text-primary">
                                {{ $post->title }}
                            </a>
                        </h3>
                        <div class="flex items-center gap-3 mt-1">
                            <span class="inline-block px-2 py-0.5 text-xs rounded-full {{ $post->status === 'published' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                                {{ $post->status === 'published' ? '已发布' : '草稿' }}
                            </span>
                            @if($post->category)
                                <span class="text-xs text-steel">{{ $post->category->name }}</span>
                            @endif
                            <span class="text-xs text-steel">{{ $post->created_at->format('Y-m-d') }}</span>
                        </div>
                    </div>
            ...
            @empty
                <div class="p-8 text-center">
                    <p class="text-steel mb-4">还没有写过文章</p>
